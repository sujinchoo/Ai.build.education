...
## 학습 내용 (목표 연계)
- **MVP 범위 설정**: 사용자가 가장 먼저 가치를 느끼는 핵심 기능 1~2개에 집중하고 나머지는 제외한다.
- **API 연동 안정성**: 타임아웃, 빈 응답, 속도 제한(rate limit) 등 실패 상황을 고려해 예외 처리를 설계한다.
- **보안 기본 원칙**: API 키는 환경변수로 분리하고, 입력 검증/로그 마스킹으로 민감정보 노출을 줄인다.
- **초급자 포인트**: ‘정상 동작’뿐 아니라 ‘실패 시 사용자에게 보여줄 메시지’까지 구현해야 서비스 완성도가 높아진다.
